...
def visualize_roc_curves(
    roc_results_all_folds,
    dataset_name,
    save_folder="results/",
):
    """Visualize ROC curves for all models across folds.
    
    Args:
        roc_results_all_folds (dict): Dictionary with ROC results for each model across folds.
        dataset_name (str): Name of the dataset.
        save_folder (str): Folder path to save visualizations.
    """
    print("Visualizing ROC Curves...")
    
    # Plot 1: Individual ROC curves for each model (averaged across folds)
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.patch.set_facecolor("white")
    axes = axes.flatten()
    
    model_names = list(roc_results_all_folds.keys())
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
    
    for idx, model_name in enumerate(model_names):
        ax = axes[idx]
        
        # Filter out NaN AUROC values and plot valid ROC curves for each fold
        valid_results = [r for r in roc_results_all_folds[model_name] if not np.isnan(r["auroc"])]
        
        if not valid_results:
            # No valid results for this model - show message
            ax.text(0.5, 0.5, f"No valid ROC data for {model_name}", 
            ...
            for result in valid_results:
                ax.plot(
                    ...
                    alpha=0.3,
                    color=colors[idx],
                    label=f"Fold {result['fold'] + 1} (AUROC: {result['auroc']:.4f})"
                )
            
            # Plot the average ROC curve
            mean_auroc = np.mean([r["auroc"] for r in valid_results])
            ax.plot([0, 1], [0, 1], 'k--', lw=2, label='Random (AUROC: 0.5000)')
            
            ax.set_xlabel("False Positive Rate", fontsize=12)
            ax.set_ylabel("True Positive Rate", fontsize=12)
            ax.set_title(f"{model_name} ROC Curves\nMean AUROC: {mean_auroc:.4f}", fontsize=13, fontweight="semibold")
    ...
    fig.tight_layout()
    fig.savefig(f"{save_folder}roc_curves_{dataset_name}.png", dpi=150)
    plt.close()
    
    # Plot 2: Comparative plot with all models on one axis
    fig, ax = plt.subplots(figsize=(10, 8))
    fig.patch.set_facecolor("white")
    
    for idx, model_name in enumerate(model_names):
        # Filter out NaN AUROC values
        valid_results = [r for r in roc_results_all_folds[model_name] if not np.isnan(r["auroc"])]
        
        if not valid_results:
            continue  # Skip this model in comparison plot
        
        mean_auroc = np.mean([r["auroc"] for r in valid_results])
        
        # Average FPR and TPR across folds (interpolated at 100 points)
        ...
        for result in valid_results:
            try:
                tpr_interp = np.interp(fpr_mean, result["fpr"], result["tpr"])
                tprs.append(tpr_interp)
                aucs.append(result["auroc"])
            except Exception as e:
                print(f"  Warning: Could not interpolate ROC for {model_name} fold {result['fold'] + 1}: {e}")
                continue
        
        if not tprs:
            continue  # No valid interpolations for this model
        ...
        ax.plot(
            ...
            tpr_mean,
            color=colors[idx],
            label=f"{model_name} (AUROC: {mean_auroc:.4f})",
            linewidth=2.5,
            marker="o",
            markersize=4,
            markevery=10
    ...
    ax.set_title(f"ROC Curves Comparison on {dataset_name}", fontsize=15, fontweight="semibold", pad=15)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    ax.grid(alpha=0.3)
    ax.set_facecolor("#f7f9fc")
    ...
    fig.tight_layout()
    fig.savefig(f"{save_folder}roc_curves_comparison_{dataset_name}.png", dpi=150)
    plt.close()
    
    # Plot 3: AUROC boxplot across folds
    fig, ax = plt.subplots(figsize=(10, 6))
    fig.patch.set_facecolor("white")
    
    auroc_data = [
        [r["auroc"] for r in roc_results_all_folds[model_name] if not np.isnan(r["auroc"])]
        for model_name in model_names
    ]
    ...
    ax.set_title(f"AUROC Distribution Across Folds on {dataset_name}", fontsize=15, fontweight="semibold", pad=15)
    ax.set_ylim([0, 1.05])
    ...
    for i, model_name in enumerate(model_names):
        valid_aurocs = auroc_data[i]
        if valid_aurocs:
            mean_auroc = np.mean(valid_aurocs)
            ax.text(i+1, mean_auroc + 0.02, f'{mean_auroc:.4f}', ha='center', fontsize=10, fontweight="semibold")
    
    fig.tight_layout()
    fig.savefig(f"{save_folder}roc_auroc_boxplot_{dataset_name}.png", dpi=150)
    plt.close()
    
    print(f"ROC visualizations saved to {save_folder}")
...
if __name__ == "__main__":
    ...
    # Initialize ROC results accumulator
    roc_results_all_folds = {
        "PlackettLuce": [],
        "MallowsModel": [],
        "PreferenceModel": [],
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        ### Compute Accuracy/Rejection Rate of Rankings Models based on uncertainty of the distribution. ###
        dist_x_pl = np.array(
            list(distribution_pl.values()), dtype=np.float32
        ).T # shape (n_samples, n_rankings)
        dist_x_mallows = np.array(
            list(distribution_mallows.values()), dtype=np.float32
        ).T # shape (n_samples, n_rankings)
        dist_x_pref = np.array(
            list(distribution_pref.values()), dtype=np.float32
        ).T # shape (n_samples, n_rankings)
        dist_x_rpc_pl = np.array(
            list(distribution_rpc_pl.values()), dtype=np.float32
        ).T # shape (n_samples, n_rankings)
        uncertainty_pl = entropy(dist_x_pl, base=2, axis=1)
        uncertainty_mallows = entropy(dist_x_mallows, base=2, axis=1)
        uncertainty_pref = entropy(dist_x_pref, base=2, axis=1)
        uncertainty_rpc_pl = entropy(dist_x_rpc_pl, base=2, axis=1)
        pred_pl = np.array(
            [possible_rankings[np.argmax(dist_x_pl[i])] for i in range(dist_x_pl.shape[0])]
        )
        pred_mallows = np.array(
            [possible_rankings[np.argmax(dist_x_mallows[i])] for i in range(dist_x_mallows.shape[0])]
        )
        pred_pref = np.array(
            [possible_rankings[np.argmax(dist_x_pref[i])] for i in range(dist_x_pref.shape[0])]
        )
        pred_rpc_pl = np.array(
            [possible_rankings[np.argmax(dist_x_rpc_pl[i])] for i in range(dist_x_rpc_pl.shape[0])]
        )
        
        ### ROC/AUROC Analysis based on Uncertainty ###
        
        # Convert y_test to ranking format for comparison
        # y_test can be either: (1) rank-per-item format (2D array), (2) order tuples (list of lists)
        # or (3) indices into possible_rankings (1D array)
        y_test_np = y_test if isinstance(y_test, np.ndarray) else y_test_tensor.numpy()
        ...
        if dataset_name in [
            ...
            # y_test is rank-per-item; convert to order tuples
            y_test_rankings = _ranks_to_order_np(y_test_np)
        else:
            # y_test is likely rank-per-item format for synthetic data too
            # Check shape to determine format
            if y_test_np.ndim == 2 and y_test_np.shape[1] == n_items:
                # Rank-per-item format: convert to order tuples
                y_test_rankings = _ranks_to_order_np(y_test_np)
            else:
                # 1D indices into possible_rankings (shouldn't happen for synthetic, but handle it)
                y_test_rankings = np.array([possible_rankings[int(idx)] for idx in y_test_np])
        
        # Compute correctness labels for each model (1 if prediction matches true ranking, 0 otherwise)
        y_true_pl = np.array([1 if np.array_equal(pred_pl[i], y_test_rankings[i]) else 0 for i in range(len(pred_pl))])
        y_true_mallows = np.array([1 if np.array_equal(pred_mallows[i], y_test_rankings[i]) else 0 for i in range(len(pred_mallows))])
        y_true_pref = np.array([1 if np.array_equal(pred_pref[i], y_test_rankings[i]) else 0 for i in range(len(pred_pref))])
        y_true_rpc_pl = np.array([1 if np.array_equal(pred_rpc_pl[i], y_test_rankings[i]) else 0 for i in range(len(pred_rpc_pl))])
        
        # Invert uncertainty to get confidence scores (for ROC curve interpretation)
        # Lower uncertainty = higher confidence = higher y_score
        max_entropy = np.log2(len(possible_rankings))  # Maximum entropy for binary classification between correct/incorrect
        confidence_pl = max_entropy - uncertainty_pl
        confidence_mallows = max_entropy - uncertainty_mallows
        confidence_pref = max_entropy - uncertainty_pref
        confidence_rpc_pl = max_entropy - uncertainty_rpc_pl
        
        # Helper function to safely compute AUROC with edge case handling
        def compute_auroc_safe(y_true, y_score, model_name=""):
            # Check for edge cases: all 0s or all 1s in y_true (no positive or negative samples)
            if len(np.unique(y_true)) < 2:
                # All same class - undefined ROC curve, return neutral AUROC and dummy FPR/TPR
                print(f"  Warning: {model_name} has uniform y_true (all same class) - using default AUROC=0.5")
            ...
            try:
                ...
                # Check if AUROC is NaN and fallback to 0.5
                if np.isnan(auroc):
                    print(f"  Warning: {model_name} AUROC computation resulted in NaN - using default AUROC=0.5")
                    return np.array([0., 1.]), np.array([0., 1.]), 0.5
                
                return fpr, tpr, auroc
            except Exception as e:
                print(f"  Warning: {model_name} ROC computation failed ({e}) - using default AUROC=0.5")
                return np.array([0., 1.]), np.array([0., 1.]), 0.5
        
        # Compute ROC curves and AUROC with safe handling
        try:
            fpr_pl, tpr_pl, auroc_pl = compute_auroc_safe(y_true_pl, confidence_pl, "PlackettLuce")
            fpr_mallows, tpr_mallows, auroc_mallows = compute_auroc_safe(y_true_mallows, confidence_mallows, "MallowsModel")
            fpr_pref, tpr_pref, auroc_pref = compute_auroc_safe(y_true_pref, confidence_pref, "PreferenceModel")
            fpr_rpc_pl, tpr_rpc_pl, auroc_rpc_pl = compute_auroc_safe(y_true_rpc_pl, confidence_rpc_pl, "PlackettLuceRPC")
            
            print("> ROC/AUROC Results:")
            print(f"PlackettLuce AUROC: {auroc_pl:.4f}")
            print(f"MallowsModel AUROC: {auroc_mallows:.4f}")
            print(f"PreferenceModel AUROC: {auroc_pref:.4f}")
            print(f"PlackettLuceRPC AUROC: {auroc_rpc_pl:.4f}")
            
            # Store ROC data for later visualization/analysis
            roc_results = {
                "PlackettLuce": {"fpr": fpr_pl, "tpr": tpr_pl, "auroc": auroc_pl, "y_true": y_true_pl, "y_score": confidence_pl},
                "MallowsModel": {"fpr": fpr_mallows, "tpr": tpr_mallows, "auroc": auroc_mallows, "y_true": y_true_mallows, "y_score": confidence_mallows},
                "PreferenceModel": {"fpr": fpr_pref, "tpr": tpr_pref, "auroc": auroc_pref, "y_true": y_true_pref, "y_score": confidence_pref},
                "PlackettLuceRPC": {"fpr": fpr_rpc_pl, "tpr": tpr_rpc_pl, "auroc": auroc_rpc_pl, "y_true": y_true_rpc_pl, "y_score": confidence_rpc_pl},
            }
            
            # Accumulate ROC results across folds
            for model_name in roc_results.keys():
                roc_results_all_folds[model_name].append({
                    "fold": fold,
                    "auroc": roc_results[model_name]["auroc"],
                    "fpr": roc_results[model_name]["fpr"],
                    "tpr": roc_results[model_name]["tpr"],
                    "y_true": roc_results[model_name]["y_true"],
                    "y_score": roc_results[model_name]["y_score"],
                })
            
        except Exception as e:
            print(f"Warning: Could not compute ROC curves for fold {fold + 1}: {e}")
            roc_results = {}
    ...
    # Visualize and Save ROC Results
    ...
    # Save AUROC results to CSV
    roc_auroc_records = []
    for model_name in roc_results_all_folds.keys():
        for result in roc_results_all_folds[model_name]:
            roc_auroc_records.append({
                "model": model_name,
                "fold": result["fold"],
                "auroc": result["auroc"],
            })
    
    roc_auroc_df = pd.DataFrame(roc_auroc_records)
    roc_auroc_df.to_csv(
        f"{save_folder}roc_auroc_results_{dataset_name}.csv",
        index=False,
    )
    
    # Save detailed ROC data (FPR, TPR) for each model and fold
    for model_name in roc_results_all_folds.keys():
        for result in roc_results_all_folds[model_name]:
            fold = result["fold"]
            fpr = result["fpr"]
            ...
            roc_data.to_csv(
                f"{save_folder}roc_curve_{model_name}_fold{fold}_{dataset_name}.csv",
                index=False,
            )
    
    print(f"ROC results saved to {save_folder}roc_auroc_results_{dataset_name}.csv")
